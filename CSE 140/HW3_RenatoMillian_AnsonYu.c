...
//Contributed by Anson
void J_instruction(char mips_code[], int Op_code, int size)
{
    char Immediate[26];
    for (int i = 0; i < size; i++){
        if(i>=6){
            Immediate[i - 6] = mips_code[i];
        }
    }
...
//Contributed by Renato
void I_instruction(char mips_code[], int Op_code, int size)
{
...
//Contributed by Renato
void R_instruction(char mips_code[], int Op_code, int size)
{
    ...
    for (int i = 0; i < size; i++){
        ...
        //get bits for shamt
        if(i>=21 && i < 26){
            SHAMT[i - 21] = mips_code[i];
        }
        //get bits for function
        if(i>=26){
            FUN[i - 26] = mips_code[i];
        }
    }
    // print type
