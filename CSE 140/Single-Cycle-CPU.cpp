...
void getinfo(){
    ...
    if(op_code_num == 0){
        ...
        std::string register_name = Register_Decode(31);
        //printf("success on R type-");
        const char* cstring = register_name.data();
        printf(cstring);
        printf(" is modified to 0x%x\n",registerfile[31]);
    ...
    // for jal jump to address and ra = pc+4(the next instrution of code)
    //for jr PC=ra so PC = ALUresult
    destination = 0;
}
void writeBack(int ALUresult){
    PC = PC4;
...
void execute(std::string instruction){
    ...
    if(AlUop == "10"){
        ...
        else if (ALU_Con == "1100")
        {
            ...
            writeBack(registerfile[RS] < registerfile[RT]);
        }else if(ALU_Con =="xxxx"){
            //does nothing just goes to write Back
            writeBack(RS);
        }
    }
    if(AlUop =="00")
    ...
    if(AlUop == "xx")
    {
        //should be fine for jal and jr
        // does nothing execute just goes to writeBack 
        writeBack(address);
    }
}    
void decode(std::string instruction){
    ...
    //J type instructions
    if(op_code_num == 2 || op_code_num == 3){
        address = std::stoi(instruction.substr(6),nullptr,2);
        //shift left by 2
        address = address *4;
    }
...
int main(){
	...
	std::string fileName = "sample_part2.txt"; // Please change this to whatever file you would like to read. Scanf maybe.
    const char* cstring = fileName.data();
    printf("File name: ");
    printf(cstring);
    printf("\n");
